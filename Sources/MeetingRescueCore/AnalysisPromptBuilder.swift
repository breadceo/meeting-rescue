...
public enum AnalysisPromptBuilder {
    ...
    public static func buildPrompt(for request: AnalysisRequest) throws -> String {
        ...
        let payload = PromptPayload(
            ...
            supplementalContext: supplementalContextPayload(from: request.supplementalContextSources),
    ...
    private static func supplementalContextPayload(
        from sources: [SupplementalContextSource]
    ) -> [SupplementalContextPayload] {
        sources.sortedForPrompt().map { source in
            SupplementalContextPayload(
                id: source.id,
                kind: source.kind.rawValue,
                title: source.title,
                sourceName: source.sourceName,
                excerpt: cappedSuffix(source.excerpt, limit: 1_200),
                priority: String(source.priority.rawValue),
